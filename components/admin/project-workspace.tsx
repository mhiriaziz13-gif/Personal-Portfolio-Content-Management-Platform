...
const sectionGroups: ReadonlyArray<{ label: string; description: string; keys: readonly string[] }> = [
  { label: "Project foundation", description: "Context, problem, role and contribution.", keys: ["overview", "business_context", "problem", "role_scope", "contribution"] },
  { label: "Execution", description: "Approach, workflow, tools, deliverables and safeguards.", keys: ["approach_evidence", "approach", "workflow_architecture", "tools_technologies", "deliverables", "validation_safeguards"] },
  { label: "Results and reflection", description: "Outcomes, limits, learning and related experience.", keys: ["qualitative_outcome", "outcome_limits", "what_i_learned", "related_expertise", "related_experience"] },
];
...
export const ProjectWorkspace = (props: Props) => {
  ...
  return <section className="rounded-xl border border-white/10 bg-[#100b24]/90 p-5 shadow-xl shadow-[#2A0E61]/20">
    ...
    {props.mode === "sections" && <div className="mt-6"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-semibold text-white">Project story</h2><p className="text-sm text-gray-400">Sections are grouped by purpose. Open a section only when you need to edit it.</p></div>{(!props.structureAvailable || missing.length > 0) && <button type="button" disabled={repairing || !props.structureAvailable} onClick={() => void repair()} className="button-primary rounded-lg px-4 py-2 disabled:opacity-50">Repair section structure</button>}</div><div className="mt-5 grid gap-6">{sectionGroups.map((group) => <section key={group.label} aria-labelledby={`group-${group.keys[0]}`}><div className="mb-3"><h3 id={`group-${group.keys[0]}`} className="font-semibold text-white">{group.label}</h3><p className="text-xs text-gray-400">{group.description}</p></div><div className="grid gap-2">{props.sections.filter((section) => !section.is_archived && group.keys.includes(props.definitions.find((item) => item.id === section.definition_id)?.section_key as never)).map((section) => { const def=props.definitions.find((item)=>item.id===section.definition_id); const complete=meaningful(section); return <Link key={section.id} href={`${base}/sections/${section.id}`} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-white/10 bg-white/5 p-4 hover:border-cyan-300/30"><div><p className="font-semibold text-white">{section.title}</p><p className="mt-1 text-xs text-gray-400">{def?.is_required ? "Required" : "Optional"} · Position {section.sort_order / 10}</p></div><div className="flex gap-2 text-xs"><span className="rounded-full bg-white/10 px-2 py-1">{section.is_visible ? "Visible" : "Hidden"}</span><span className={`rounded-full px-2 py-1 ${complete ? "bg-emerald-500/15 text-emerald-100" : section.is_visible ? "bg-amber-500/15 text-amber-100" : "bg-white/10"}`}>{complete ? "Complete" : section.is_visible ? "Needs attention" : "Empty"}</span></div></Link>; })}</div></section>)}</div><p className="mt-4 text-sm text-cyan-100" aria-live="polite">{status}</p></div>}
